  ...
  try {
    ...
    if (existingLink) {
      ...
      return sendSuccess(res, 'Affiliate link already exists for this customer and event', {
        ...
        affiliateUrl: `${process.env.FRONTEND_URL || 'https://easytickets.in'}/#/events/${existingEventId}?ref=${existingLink.affiliateCode}`,
      });
    }
    ...
    try {
      ...
      const affiliateUrl = `${process.env.FRONTEND_URL || 'https://easytickets.in'}/#/events/${eventId}?ref=${affiliateLink.affiliateCode}`;
...
const getAffiliateLinkDetails = async (req, res) => {
  try {
    ...
    const eventId = affiliateLink.eventId._id || affiliateLink.eventId;
    ...
    const affiliateUrl = `${process.env.FRONTEND_URL || 'https://easytickets.in'}/#/events/${eventIdForUrl}?ref=${affiliateLink.affiliateCode}`;
  ...
  try {
    ...
    const linksWithStats = await Promise.all(
      affiliateLinks.map(async (link) => {
        ...
        const affiliateUrl = `${process.env.FRONTEND_URL || 'https://easytickets.in'}/#/events/${eventIdForUrl}?ref=${link.affiliateCode}`;
