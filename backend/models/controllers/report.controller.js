...
const getEventWiseBookings = async (req, res) => {
  try {
    ...
    return sendSuccess(res, 'Event-wise bookings fetched successfully', {
      bookings,
    });
  } catch (error) {
    console.error('Error fetching event-wise bookings:', error);
    return sendError(res, 'Failed to fetch event-wise bookings', 500);
  }
};
