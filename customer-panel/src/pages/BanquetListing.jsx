...
import { Building2, MapPin, Users, Star, Search, Filter, ChevronRight, Layout, ChevronsLeft, ChevronsRight } from 'lucide-react'
import Pagination from '../components/common/Pagination'

// Carousel Component
const BanquetCarousel = ({ banquets, navigate, getImageUrl }) => {
  const [currentIndex, setCurrentIndex] = useState(1);
  const [isTransitioning, setIsTransitioning] = useState(true);

  const length = banquets?.length || 0;

  useEffect(() => {
    if (!banquets || length === 0) return;
  ...
  const displayList = [banquets[length - 1], ...banquets, banquets[0]];
  ...
  return (
    <div className="relative w-full bg-white dark:bg-gray-900 overflow-hidden py-0 md:py-0 cursor-pointer select-none">
      ...
      >
        {displayList.map((bq, idx) => {
          if (!bq) return null;
          const bgImg = getImageUrl(bq.banners?.[0]);

          if (!bgImg) return null;

          return (
            <div
              key={`carousel-${bq._id}-${idx}`}
              onClick={() => navigate(`/banquets/${bq._id}`)}
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex items-end">
                <div className="p-4 md:p-8 w-full bg-black/10 transition-all text-left rounded-b-xl xl:rounded-b-3xl">
                  <h2 className="text-white text-xl md:text-3xl font-bold line-clamp-1 group-hover:text-primary-400 transition-colors uppercase">{bq.title}</h2>
                  <div className="flex items-center text-gray-200 mt-2 text-sm md:text-base gap-2">
                    <MapPin className="w-4 h-4 text-primary-400" />
                    <span>{bq.address?.city}, {bq.address?.state}</span>
                  </div>
                </div>
              </div>
            </div>
...
const BanquetListing = () => {
  ...
  useEffect(() => {
    const fetchCities = async () => {
      try {
        const response = await api.get('/banquets/public?limit=1000')
        if (response.data.status === 200) {
          const allBq = response.data.result.banquets || []
          const uniqueCities = [...new Set(allBq.map(bq => bq.address?.city))].filter(Boolean)
  ...
  const fetchBanquets = async () => {
    try {
      setLoading(true)
      const response = await api.get('/banquets/public', {
        params: {
          page: currentPage,
          limit: itemsPerPage,
  ...
  const carouselBanquets = banquets.filter(bq => bq.banners && bq.banners.length > 0).slice(0, 5);

  if (loading) return <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900"><Loading size="lg" text="Finding best banquets..." /></div>

  return (
    <div className="min-h-screen bg-white dark:bg-gray-900 pb-12">
      {/* BookMyShow Style Carousel */}
      <BanquetCarousel banquets={carouselBanquets} navigate={navigate} getImageUrl={getImageUrl} />
      ...
      <div className="max-w-7xl mx-auto px-4 mt-6 md:mt-8">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Search banquet by name, location or keyword..."
              value={searchTerm}
      ...
      <div className="max-w-7xl mx-auto px-4 mt-8 md:mt-12">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-gray-100">Premium Banquet Halls</h2>
            ...
            {banquets.map((bq) => (
              ...
              >
                ...
                <div className="py-2.5 flex flex-col space-y-0.5 pointer-events-none">
                  <h3 className="text-sm md:text-base font-semibold text-gray-900 dark:text-gray-100 line-clamp-1 group-hover:text-primary-600 transition-colors tracking-wide uppercase">{bq.title}</h3>

                  <div className="text-gray-500 dark:text-gray-400 text-xs md:text-sm">
                    <span className="truncate line-clamp-1">{bq.address?.city}, {bq.address?.state} • {bq.venues?.length || 0} Venues</span>
                  </div>

                  <div className="pt-1 flex items-baseline flex-wrap gap-1">
                    <span className="text-[10px] md:text-xs text-gray-500 dark:text-gray-400 mt-0.5 font-medium uppercase tracking-wide">Capacity</span>
                    <span className="text-sm md:text-base font-bold text-gray-900 dark:text-gray-100 pb-0.5">Max {Math.max(...(bq.venues?.map(v => v.floatingCapacity) || [0]), 0)} Guests</span>
                  </div>
                </div>
              </div>
            ))}
