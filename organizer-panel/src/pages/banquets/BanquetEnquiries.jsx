...
import { MessageSquare, Search, User, Phone, Mail, Clock, CheckCircle, Building2 } from 'lucide-react'
import { format } from 'date-fns'

const BanquetEnquiries = () => {
  ...
  const fetchEnquiries = async () => {
    try {
      setLoading(true)
      const response = await api.get('/banquets/organizer/enquiries')
  ...
  const updateStatus = async (id, newStatus) => {
    try {
      setUpdatingId(id)
      const response = await api.put(`/banquets/organizer/enquiries/${id}`, { status: newStatus })
      if (response.data.status === 200) {
  ...
  const filteredEnquiries = enquiries.filter(enquiry => {
    ...
    return (
      ...
      enquiry.banquetId?.title?.toLowerCase().includes(lowerCaseSearchTerm) ||
  ...
  if (loading && enquiries.length === 0) return <Loading size="lg" text="Loading banquet enquiries..." />

  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Banquet Enquiries</h1>
        <p className="text-sm text-gray-500 mt-1">Manage and respond to event inquiries and track engagement for your banquet halls</p>
      </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="flex-1">
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search by customer name, email, mobile or banquet hall..."
                value={searchTerm}
      ...
      {filteredEnquiries.length === 0 ? (
        <EmptyState
          ...
          message="Event bookings and inquiries for your banquets will appear here"
        />
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase">Banquet Hall</th>
                    ...
                    <td className="px-6 py-4 text-right">
                      {enquiry.status === 'pending' ? (
                        <button
                          ...
                          className="text-xs font-bold text-primary-600 hover:text-primary-700 bg-primary-50 hover:bg-primary-100 px-3 py-1.5 rounded transition-all shadow-sm"
                        >
                          Mark Attended
                        </button>
                      ) : (
                        <span className="text-xs font-medium text-gray-400 italic">Responded</span>
                      )}
                    </td>
                  </tr>
                ))}
...
export default BanquetEnquiries
